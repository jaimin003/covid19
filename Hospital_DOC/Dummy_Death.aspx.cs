...
public partial class Hospital_DOC_Default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {           
            cn.Open();
            qry = "update Discharge_p set status = 'DEATH' where pid='" + pid + "'";
            ...
            cn.Close();
             ...
             cn.Open();
             qry = "select * from Discharge_p where status='DEATH' and  hname='" + Session["hname"].ToString() + "'";
            ...
            adp.Fill(ds);
            cn.Close();
            int death_tot = ds.Tables[0].Rows.Count;
            ...
            if (flag == 1)
            {
                ...
                cn.Open();
                qry = "update total set total_death='" + death_tot + "' where hname='" + Session["hname"] + "'";
                cmd = new SqlCommand(qry, cn);
                cmd.ExecuteNonQuery();
                cn.Close();
            ...
            {
                cn.Open();
                qry = "insert into total values('" + Session["hname"].ToString() + "','" + sname + "','" + cname + "','0','0','0','" + total_active + "','0')";
                cmd = new SqlCommand(qry, cn);
                cmd.ExecuteNonQuery();
                flag = 1;
                cn.Close();
        ...
        {
            cn.Open();
            qry = "update Discharge_p set status = 'DEATH' where pid='" + Session["pid"] + "'";
            cmd = new SqlCommand(qry, cn);
            cmd.ExecuteNonQuery();
            cn.Close();
            ...
            {
                cn.Open();
                qry = "insert into total values('" + Session["hname"].ToString() + "','" + sname + "','" + cname + "','0','0','" + death_tot + "','0','0')";
                cmd = new SqlCommand(qry, cn);
                cmd.ExecuteNonQuery();
                flag = 1;
                cn.Close();
