...
public partial class Hospital_DOC_Default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        cn.Open();
        qry = "select * from VaccineBook_mstr where status = 0 and hname='"+Session["hname"]+"'";
        ...
        for (int i = 1; i <= s; i++)
        {
            cn.Close();
            cn.Open();
            qry = "select * from VaccineBook_mstr where status = 0 and bid='" + tbid + "' and hname='" + Session["hname"] + "'";
            cmd = new SqlCommand(qry, cn);
            dr = cmd.ExecuteReader();
            if (dr.HasRows)
            {
                ...
                if (date1 < edate)
                {
                    ...
                    cn.Open();
                    qry = "delete from VaccineBook_mstr where bid='" + tbid + "' and hname='" + Session["hname"] + "'";
                    cmd = new SqlCommand(qry, cn);
                    cmd.ExecuteReader();
                    ...
                    //string Body = "DEAR " + pname1 + ", YOUR VACCINATION SCHEDULED IS EXPIRED DUE TO YOUR ABSENCE.";
                    mail.Body = CBody();
