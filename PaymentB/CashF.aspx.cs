...
public partial class PaymentB_CashF : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            cn.Close();
            cn.Open();
            qry = "select * from Bed_Book where pname='" + pname1 + "' and email='" + email1 + "'";
            cmd = new SqlCommand(qry, cn);
            dr = cmd.ExecuteReader();
            if (dr.HasRows)
            {
            ...
            {
                Response.Redirect("../UserCovid/ntbbed.aspx");
            }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        cn.Close();
        cn.Open();
        qry = "insert into Bpaid values('" + pid + "','" + pname1 + "','" + email1 + "','" + phno1 + "','" + age1 + "','" + hname1 + "','" + tday1 + "','" + tamt1 + "','CASH','0','" + DateTime.Now.ToString("dd-MM-yyyy") + "')";
        cmd = new SqlCommand(qry, cn);
        cmd.ExecuteNonQuery();
        cn.Close();
