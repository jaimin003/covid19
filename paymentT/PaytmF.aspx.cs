...
public partial class paymentT_PaytmF : System.Web.UI.Page
{
    ...
    protected void btn_paytm_Click(object sender, EventArgs e)
    {
            cn.Close();
        cn.Open();
        qry = "insert into Tpaid values('" + id + "','" + pname1 + "','" + email1 + "','" + bldgrp1 + "','" + hname1 + "','" + Convert.ToInt32(tamt) + "','PAYTM','" + DateTime.Now.ToString("dd-MM-yyyy") + "','1')";
        cmd = new SqlCommand(qry, cn);
        cmd.ExecuteNonQuery();
        cn.Close();
